...
> Euler plots are a great exploratory tool to visualize shared ancestries. Combinations increase exponentially, so care should be given when choosing a starting set. We're limiting our groups today to eight ancestries with all students sharing some AI/AN ancestry. While errors increase with the number of combinations, there is valuable high-level insight to the more complex figures. You'll notice the notable stubbornness in the examples below of the plots' refusal to visualize students with combined Black & Hispanic ancestry. Our recommendation is to use Euler plots to explore data, whereas UpSet plots may be a better tool for sharing out information. 
...
<div class="alert alert-dismissible alert-warning">
  ...
  <strong>Heads up!</strong> Increasing the number of variables will lead to errors and missing data. Be sure to match the sum of the combinations shown with your total headcount. 
</div>
...
We've increased the possible number of combinations, 7 for three elements ($2^3 -1$), in the first plot to 255 combinations for 8 elements ($2^8-1$). That said, not all combinations are used. Notably, with all students sharing AI/AN ancestry we won't have any other single ancestry groups (i.e. only AA/Black, only Asian, and so on). With the given data design, MENA will always be nested with White as well. 
